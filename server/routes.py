...
def read_file(file: UploadFile, skipOneLine):
...
async def agrisolum(file: UploadFile = File(...), destLab=""):
    ...
    if not filename.endswith((".csv", ".xlsx", ".xls")):
        response = JSONResponse(status_code=HTTP_400_BAD_REQUEST, content={
    ...
    try:
        ...
        openFile = read_file(file, True)
        if destLab == "inceres":
            df_formatado = agrisolum2inceres(openFile)
        elif destLab == "datafarm":
            df_formatado = agrisolum2datafarm(openFile)
        else:
            response = JSONResponse(status_code=HTTP_403_FORBIDDEN, content={
                                    "error": "Laboratorio final nao existe"})
